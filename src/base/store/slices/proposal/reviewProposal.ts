...
export const {requestStart, requestSuccess, requestFailure} = proposalReviewSlice.actions;
export default proposalReviewSlice.reducer;

export const reviewProposal = (dataModel: any): AppThunk => async (dispatch) => {
    try {
        dispatch(requestStart());
        const axiosInstance = getAxiosInstance();
        const response = await axiosInstance.post('/proposal/review', {
            ProposalId: dataModel.ProposalId,
            IsApproved: dataModel.IsApproved,
            RejectReason: dataModel.RejectReason
        });
