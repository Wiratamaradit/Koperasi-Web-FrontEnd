...
export const {requestStart, requestSuccess, requestFailure} = productCreateSlice.actions;
export default productCreateSlice.reducer;

export const productCreate = (dataModel: any): AppThunk => async (dispatch) => {
    try {
        dispatch(requestStart());
        const axiosInstance = getAxiosInstance();
        const response = await axiosInstance.post('/product', {
            // MediaId: dataModel.MediaId,
            ProductProfileId: dataModel.ProductProfileId.code,
            ProductSegmentId: dataModel.ProductSegmentId.code,
            Code: dataModel.Code,
            Name: dataModel.Name,
