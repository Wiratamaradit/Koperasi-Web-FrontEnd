...
export const {requestStart, requestSuccess, requestFailure} = customerCreateSlice.actions;
export default customerCreateSlice.reducer;

export const customerCreate = (dataModel: any): AppThunk => async (dispatch) => {
    try {
        dispatch(requestStart());
        const axiosInstance = getAxiosInstance();
        const response = await axiosInstance.post('/customer', {
            MediaId: dataModel.MediaId,
            ShipPartyId: dataModel.ShipPartyId,
            CustomerSegmentId: dataModel.CustomerSegmentId,
            Code: dataModel.Code,
            Name: dataModel.Name,
