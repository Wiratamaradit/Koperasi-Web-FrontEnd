...
export const shipPartyGroupUpdateSlice = createSlice({
    name: 'shipPartyGroupPut',
...
export const {requestStart, requestSuccess, requestFailure} = shipPartyGroupUpdateSlice.actions;
export default shipPartyGroupUpdateSlice.reducer;

export const shipPartyGroupUpdate = (dataModel: any): AppThunk => async (dispatch) => {
    try {
        dispatch(requestStart());
        const axiosInstance = getAxiosInstance();
        const response = await axiosInstance.put('/ship-party-group/' + dataModel.Id, {
