...
export const roleSchemeCreateSlice = createSlice({
    name: 'roleSchemePost',
...
export const {requestStart, requestSuccess, requestFailure} = roleSchemeCreateSlice.actions;
export default roleSchemeCreateSlice.reducer;

export const roleSchemeCreate = (dataModel: any): AppThunk => async (dispatch) => {
    try {
        dispatch(requestStart());
        const axiosInstance = getAxiosInstance();
        const response = await axiosInstance.post('/approval-scheme', {
            Name: dataModel.Name,
...
export const roleSchemeResetCreateState = (): AppThunk => async (dispatch) => {
    dispatch(requestStart());
